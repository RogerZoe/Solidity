...
contract Project {
    ...
    function CreateEvent( string memory name,uint256 price,uint256 date,uint256 Ticketcount) public {
        // NEED TO CHECK THE CONDITIONS...
       
        require( date > block.timestamp,"You can organize event for future date"); // check will happens with seconds unix time.. not for the past
        require( Ticketcount > 0,"You can organize event only if you create more than 0 tickets"); // condtion to to create an event

        events[nextID++] = Event(name,msg.sender,price,date,Ticketcount,Ticketcount);
    }
    ...
    function TransferTicket(uint  id,address to,uint Quantity) external
    {
               

                require(events[id].date!=0,"Event does not exist");
                require(events[id].date>block.timestamp,"Event has already occured");
                require(tickets[msg.sender][id]>=0," NOT ENOUGH TICKETS TO TRANSFER");
